...
const API_BASE_URL = "https://cyber360-api.onrender.com/usuarios";
...
const mapFrontendToBackend = (usuario) => ({
  ...
  nombre: usuario.name,
  celular: usuario.telefono, // estandarizado: frontend usa "telefono"
  email: usuario.email,
  direccion: usuario.direccion,
  fkRol: usuario.rolId, // se manda el id del rol
  contrasena: usuario.contrasena || null, // opcional en update
});
...
export const toggleUsuarioEstado = async (id, estadoActual) => {
  try {
    ...
    // Llamada PATCH al endpoint /estado enviando el estado en el body
    await axios.patch(`${API_BASE_URL}/${id}/estado`, { estado: nuevoEstado });

    return nuevoEstado ? "activo" : "inactivo";
  } catch (error) {
